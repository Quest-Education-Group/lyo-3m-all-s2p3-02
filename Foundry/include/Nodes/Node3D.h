...
public: 
	...
	const glm::vec3& GetRight()					{ return m_transform.GetRight(); }
	const glm::vec3& GetUp()					{ return m_transform.GetUp(); }
	const glm::vec3& GetForward()				{ return m_transform.GetForward(); }
	...
	void SetRotation(glm::vec3 rot)				{ m_transform.SetRotation(rot); }
	...
	void AddPosition(glm::vec3 pos)				{ m_transform.AddPosition(pos); }
	void AddX(float x)							{ m_transform.AddX(x); }
	void AddY(float y)							{ m_transform.AddY(y); }
	void AddZ(float z)							{ m_transform.AddZ(z); }
	void AddRotation(glm::vec3 rot)				{ m_transform.AddRotation(rot); }
	...
	void AddScale(glm::vec3 scale)				{ m_transform.AddScale(scale); }
